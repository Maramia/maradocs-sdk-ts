...
export const PageNumberSchema = z
  ...
  .int()
  .min(0)
  .describe("Zero-indexed page number");
export type PageNumber = z.infer<typeof PageNumberSchema>;

/**
 * DPI for PDF image operations (30-600)
 */
export const PdfImgDpiSchema = z
  .number()
  .int()
  .min(30)
  .max(600)
  .describe("DPI for PDF images (30-600)");
 ...
 */
export const PdfValidateRequestSchema = z
  .object({
    unvalidated_file_handle: UnvalidatedFileHandleSchema.describe(
      "Handle to the unvalidated PDF file",
    ),
    password: z
      .string()
      .optional()
      .describe("Password to decrypt the PDF (if encrypted)"),
  })
 ...
 */
export const PdfValidateResponseOkSchema = z
  .object({
 ...
 */
export const PdfValidateResponseErrorSchema = z
  .object({
    class_name: z.literal("PdfValidateResponseError"),
    error: z.string().describe("Error message"),
  })
  .describe("PDF validation error");
export type PdfValidateResponseError = z.infer<
  typeof PdfValidateResponseErrorSchema
>;

/**
 * PDF validation failed due to virus detection
 */
export const PdfValidateResponseVirusSchema = z
  .object({
...
export type PdfValidateResponseVirus = z.infer<
  typeof PdfValidateResponseVirusSchema
>;

/**
 * Response to a PDF validation request (discriminated union)
 */
export const PdfValidateResponseSchema = z
  .object({
 ...
 */
export const PdfComposePdfPageSchema = z
  .object({
    page_number: PageNumberSchema.describe(
      "Zero-indexed page number to include",
    ),
    rotation: DiscreteAngleSchema.default(0).describe(
      "Rotation angle for this page. Default: 0",
    ),
  })
 ...
 */
export const PdfComposePdfSchema = z
  .object({
    pdf_handle: PdfHandleSchema.describe("Handle to the source PDF"),
    pages: z
      .array(PdfComposePdfPageSchema)
      .optional()
      .describe("Pages to include. If not provided, all pages are used."),
  })
 ...
 */
export const PdfComposeRequestSchema = z
  .object({
 ...
 */
export const PdfComposeResponseSchema = z
  .object({
 ...
 */
export const PdfRotateRequestSchema = z
  .object({
    pdf_handle: PdfHandleSchema.describe("Handle to the PDF to rotate"),
    rotate: z
      .array(z.tuple([PageNumberSchema, DiscreteAngleSchema]))
 ...
 */
export const PdfRotateResponseSchema = z
  .object({
 ...
 */
export const PdfOptimizeRequestSchema = z
  .object({
    pdf_handle: PdfHandleSchema.describe("Handle to the PDF to optimize"),
    image_dpi: PdfImgDpiSchema.default(150).describe(
      "Target DPI for images. Default: 150",
    ),
    image_quality: PdfImgQualitySchema.default(70).describe(
      "Image quality (1-100). Default: 70",
    ),
    image_color: PdfImgColorSchema.default("original").describe(
 ...
 */
export const PdfOptimizeResponseSchema = z
  .object({
 ...
 */
export const PdfOcrToPdfRequestSchema = z
  .object({
 ...
 */
export const PdfOcrToPdfResponseSchema = z
  .object({
 ...
 */
export const PdfOrientationRequestSchema = z
  .object({
 ...
 */
export const PdfOrientationResponseSchema = z
  .object({
    orientations: z
      .array(DiscreteAngleSchema)
      .describe("List of orientations for each page"),
    rotated_pdf_handle: PdfHandleSchema.describe(
      "PDF with pages auto-rotated to correct orientation",
    ),
  })
  .describe("PDF orientation detection response");
export type PdfOrientationResponse = z.infer<
  typeof PdfOrientationResponseSchema
>;
 ...
 */
export const PdfToImgRequestSchema = z
  .object({
    pdf_handle: PdfHandleSchema.describe("Handle to the PDF to render"),
    pages: z
      .array(PageNumberSchema)
      .optional()
      .describe("Pages to render. If not provided, all pages are rendered."),
    dpi: z
      .number()
      .int()
      .min(72)
      .max(600)
      .default(200)
      .describe("DPI for rendering. Default: 200"),
  })
 ...
 */
export const PdfToImgResponseSchema = z
  .object({
    img_handles: z
      .array(ImgHandleSchema)
      .describe("List of image handles, one per rendered page"),
  })
...
/**
 * Utility function to extract the PDF handle from a validation response.
 * Returns the PDF handle if validation was successful.
 ...
 * @param response - The PDF validation response
 * @returns The validated PDF handle
 * @throws {ValidationErrorException} If validation failed due to an error
 * @throws {ValidationVirusException} If a virus was detected
 */
export function okPdf(response: PdfValidateResponse): PdfHandle {
  if (response.response.class_name === "PdfValidateResponseOk") {
    return response.response.pdf_handle;
  } else if (response.response.class_name === "PdfValidateResponseError") {
    throw new ValidationErrorException(response.response.error);
  } else if (response.response.class_name === "PdfValidateResponseVirus") {
